    ...
    <van-pull-refresh v-model="refreshing" @refresh="onRefresh">
        <van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad" :error.sync="error" error-text="请求失败，点击重新加载">
            <van-cell v-for="item in list" :key="item.art_id" :title="item.title">
              <!-- 三张封面是显示内容-->
                <van-row slot="label" v-if="item.cover.images&&item.cover.type===3">
                    <van-col span="8" v-for="(pic,index) in item.cover.images" :key="index">
                        <van-image width="127" height="70" :src="pic" />
                    </van-col>
                </van-row>
                <!-- 一张图片时候显示 -->
                <van-image v-else-if="item.cover.images&&item.cover.type===1" slot="default" width="127" height="70" :src="item.cover.images[0]" />
            </van-cell>
        </van-list>
    </van-pull-refresh>
</div>
